...
Each of the three coordinates is discretized in a way that preserves $u := 20$ meter-precision in the worst case.
With the semi-major axis $a$, the maximum circumference of the ellipsoid equals $=2 a \pi$~meters.
Using base two, $B_{x}:= \left\lceil \log_2(\frac{2 a \pi}{u}) \right\rceil = 21$ bits for the longitude thus suffice to represent each possible meter value in the worst case.
...
Thus $\left\lceil\log_2(\frac{b \pi}{u})\right\rceil = 20 =: B_{y}$ bits suffice.
...
Covering this range requires $B_{z} := \left\lceil \log_2(\frac{H - D}{u}) \right\rceil = 11$ bits.
In fact using $B_z$ bits starting at $D$ will cover the range from $D$ to $D + (2^{B_{z}} - 1) \cdot u = 30'940 := H$.
...
Each leaf represents a full discretized coordinate using $B := \cdot B_{x} + B_{y} + B_{z} = 52$ bits resulting in a total of $2^{B}$ leaves.
...
Thus in the worst case a proof for a single leaf consists $\left(B-1\right) \cdot 32$~B $= 51 \cdot 32$~B $= 1'632$~B $\approx 1.6$~KiB and is thus slightly smaller than the average X.509 certificate ($\approx 2$~KiB).
...
\begin{itemize}
	\item \texttt{bit\_string} of type \texttt{bit varying (52)}
...
For simplicity let's assume $B_{y} = B_{x} = 21 := B_{x,y}$ in the following analysis.
...
For the hybrid-quadtree, the tree height would be $\log_4(2^{2 \cdot B_{x,y}})= \log_4(4^{B_{x,y}}) = B_{x,y} = 21$ plus the $\log_2(2^{B_z}) = B_z = 11$ for the binary subtrees.
For the hybrid-octree the tree height would be
$\log_8(2^{3 \cdot B_z}) = \log_8(8^{B_z}) = B_z = 11$ plus the $\lceil \log_4(2^{B - 3 \cdot B_z}) \rceil = 10$ for the quad-subtrees.
The proof size of a leaf would thus increase to $(21 \cdot (4 - 1) + (11 - 1) \cdot (2 - 1)) \cdot 32$~B $= 2'336$~B $\approx$ $2$~KiB and $(11 \cdot (8 - 1) + (10 - 1) \cdot (4 - 1)) \cdot 32$~B $= 3'328$~B $\approx$ $3$~kB, respectively.
