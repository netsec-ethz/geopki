...
For the latitude the circumference is $C_{y} :=2 b \pi$~meters and thus $B_{y} := \left\lceil \log_2(C_{y}) \right\rceil = B_{x} =: B_{x,y}$ meaning the same minimum number of bits are required.
...
\ck{used same figure twice and numbers don't seem correct given the size of the circle}
...
An alternative design could split the trees and use a standard append-only MHT for the proofs and a tree similar to the one described before solely for indexing.
This has the advantage that the proofs of consistency are more efficient but comes with the downside that spatially close locations will no longer have an increased likelihood of being close together in the tree.
Hence the expected number of shared ancestors would be smaller and the optimizations described in \Cref{sec:optimizations} would be less effective.
Should empirical data indicate that the savings resulting from said optimization are minimal, this design option should be considered.
\ck{how would a proof of absence work in this design? I guess the proof of absence be done in the index tree, right? Then we would still need to provide an MHT-like index tree with signed root, so it's not clear to me what the benefit then would be.}

Another alternative design could use a (hybrid-)quadtree or (hybrid-)octree where most intermediate nodes have four or eight children, respectively.
