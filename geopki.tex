...
Alternatively, the first $B_{x} + B_{y}$ steps can be performed by intersecting a circle (at the height of the sphere's center where the radius is maximized) with rectangles (the voxels areas at zero elevation).
Analogously for the last $B_z$ steps, the sphere's and voxels' projections to the $z$ axis can be intersected.
Note that this over-approximates the result since a voxel is counted as intersecting if it intersects the the circle extruded between its limiting $z$ coordinates, i.e. the corresponding cylinder.
...
A response needs to include all intermediate and leaf nodes whose spatial representation intersects the queried sphere.
...
PostgreSQL's postgis extension has a \texttt{ST\_DWithin} that supports polar coordinates and a radius in meters but it only operates in two dimensions and the three-dimensional equivalent \texttt{ST\_3DDWithin} does not support polar coordinates.
Simply treating the polar coordinates as if they were cartesian will cause wrong results.
Hence using an actual three-dimensional sphere seems infeasible but it could be over-approximated using a polyhedron and check for intersections.
Unfortunately as in the case of the three-dimensional intersection function \texttt{ST\_3DIntersects} also does not support polar coordinates.
Thus directly using a three-dimensional spatial index does not seem to be possible when using polar coordinates.
Falling back to two dimensions, \texttt{ST\_DWithin} and \texttt{ST\_Intersects} both support polar coordinates which enables a similar approach as previously described:
First one predicate ensures the intersection with the circle defined by sphere's center and it's radius using \texttt{ST\_DWithin} or \texttt{ST\_Intersects}.
In the case of \texttt{ST\_Intersects}, the circle needs to be approximated using a polygon as circles cannot be represented solely using polar coordinates.
A second predicate ensures the $z$ coordinate is within a given sphere's range.
As previously noted this approximates the sphere as a cylinder and in case of \texttt{ST\_DWithin} will result in more data returned than actually requested.
When using \texttt{ST\_Intersects}, things get a bit more complicated as over-approximating a circle in polar coordinates does not seem to be a trivial task.
Hence in this case it would probably be best to let the client perform the approximation to prevent the server accidentally omitting data due to approximation errors.
...
An alternative approach would be to limit a input sphere's radius to a value where the differences between polar and cartesian coordinates are not significant and thus \texttt{ST\_3DDWithin} or \texttt{ST\_3DIntersects} could be used.
The risk is that this results in under-approximating the data which would make the server seemingly behave malicious.

MySQL has equivalent functions for two dimensional data but seemingly not for three dimensional data.
Given the above, a cartesian coordinate system as proposed in the original GeoPKI \cite{kim_geopki_2013} paper would certainly be easier to query as \texttt{ST\_3DDWithin} could be used.

\subsection{Under-approximating a Circle using Polar Coordinates}

If \texttt{ST\_Intersects} is used, it is required to approximate the circle using a polygon.
...
The resulting set of points define a polygon that under-approximates the circle.
See \Cref{fig:sphere-approximation} for examples how this shape can then be extruded to under-approximate a sphere.
