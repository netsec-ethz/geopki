...
\DeclareMathAlphabet{\mymathbb}{U}{BOONDOX-ds}{m}{n} % mathbb 0 and one
\newcommand{\mathbbzero}{\mymathbb{0}}
\newcommand{\mathbbone}{\mymathbb{1}}
...
WGS84 defines the \emph{semi-major axis} (half of the largest diameter of an ellipsoid, at the equator) $r_a$ to be $6'378'137.0$~meters \cite{noauthor_nga_nodate}. The \emph{semi-minor axis} (the polar radius) $r_b$ is defined as $6'356'752.3142~$meters \cite{noauthor_nga_nodate}.
...
With the semi-major axis $r_a$, the maximum circumference of the ellipsoid equals $=2 \cdot r_a \cdot \pi$~meters.
Using base two, $B_{x}:= \left\lfloor \log_2(\frac{2 \cdot r_a \cdot \pi}{u}) \right\rfloor + 1 = 26$ bits for the longitude thus suffice to represent each possible meter value in the worst case.
Let $C_{x} := 2^{B_{x}} - 1$ denote the maximum discretized value for the longitude.
For the latitude the circumference is $2 b \pi$~meters but we only need to be able to represent half of it, so $r_b \cdot \pi$.
Thus $\left\lfloor\log_2(\frac{r_b \cdot \pi}{u})\right\rfloor + 1 = 25 =: B_{y}$ bits suffice.
...
Covering this range requires $B_{z} := \left\lfloor \log_2(\frac{H - D}{u}) \right\rfloor + 1 = 15$ bits.
...
Each node is encoded using a bit string pair from the set $\mathcal{N}$ defined as follows:
\begin{equation}
	\mathcal{N} := \bigcup_{\substack{i \in \{0, 1, ..., B_x + B_y]\\j \in \{0, 1, ..., B_z\}}} \{0, 1\}^{i} \times \{0, 1\}^{j}
\end{equation}

where $\mathcal{N}_r := \left(\epsilon, \epsilon \right)$ is the root of the tree with $\epsilon$ representing the empty bit string.
Let $\mathcal{C}: \mathcal{N} \mapsto \mathcal{P}\left(\mathcal{N}\right)$ be the function mapping each node to its children in the SMT where $\mathcal{P}$ denotes the power set.
Moreover let $||$ denote the concatenation operator.
Formally $\mathcal{C}(n)$ for some node $n = (n_{0}, n_1)$ is then defined as follows:
%\begin{equation}
%	C(b_0, b_1) = \left\{
%	\begin{array}{ll}
%	\{(b'_0, b'_1) \in \mathcal{N} \: | \: \} & \text{if $b_z = \epsilon$}\\
%	\{(b'_0, b'_1) \in \mathcal{N} \: | \: & \text{if $b_z \neq \epsilon$}\\
%	\left(b'_1 = b_1 || 0 \right) \lor \left(b'_1 = b_1 || 1 \right) \} &
%	\end{array}
%	\right.
%\end{equation}
...
%For $b_1 = \epsilon$ :
%\begin{align*}
%	\mathcal{C}\left(\left(b_0, b_1\right)\right) :=\:& \{(b'_0, b_1) \in \mathcal{N} \: | \: b'_0 \in \left\{b_0 || 0, \: b_0 || 1\right\} \\\
%	\cup \: & \{(b_0, b'_1) \in \mathcal{N} \: | \: b'_1 \in \left\{b_1 || 0,\: b_1 || 1\right\}
%\end{align*}
%
%For $b_1 \in \bigcup_{j \in \{1,2,...,B_z-1\}} \{0,1\}^{j}$ :
%\begin{align*}
%	\mathcal{C}\left(\left(b_0, b_1\right)\right) := \{(b_0, b'_1) \in \mathcal{N} \: | \left(b'_1 = b_1 || 0 \right) \lor \left(b'_1 = b_1 || 1 \right) \}
%\end{align*}

For $n_1 = \epsilon$ :
\begin{align*}
	\mathcal{C}\left(\left(b_0, b_1\right)\right) :=\:& \left\{(b_0 || 0, n_1), (n_0 || 1, n_1)\right\}\\
	\cup & \left\{(n_0, b_1 || 0), (n_0, b_1 || 1) \right\}
\end{align*}

For $n_1 \in \bigcup_{j \in \{1,2,...,B_z-1\}} \{0,1\}^{j}$ :
\begin{align*}
	\mathcal{C}\left(\left(n_0, n_1\right)\right) :=\:& \left\{(n_0, n_1 || 0), (n_0, n_1 || 1) \right\}
\end{align*}

And for leafs ($n_1 \in \{0,1\}^{B_z}$)
\begin{align*}
	C\left(\left(n_0, n_1\right)\right) := \{ \}
\end{align*}

In words the first entry in the pair encodes the $x$ (longitude) and $y$ (latitude) coordinates and the second one the $z$ (altitude).
Let \emph{2D bit string} denote the first entry of a pair and \emph{$z$ bit string} the second one.
% Moreover let \emph{left child} denote the bit string obtained by appending a zero and \emph{right child} the one obtained by appending a one.
Each node with an empty $z$ bit string has exactly four children:
Two with an the empty string as their $z$ bit string where the 2D bit strings are obtained by appending a zero and a one to the parent's.
And another two children with the same 2D bit strings as the parent's but a zero and a one as their $z$ bit string.
Each node with a non-empty $z$ bit string has exactly two children that have are obtained by appending a zero and a one to the parent's $z$ bit string. The 2D bit string is equal to the parent's for both children.

To have spatially close points somewhat close together in the SMT, the space-filling $Z$-order curve is used to sort the two dimensional locations in one dimension \cite{morton_computer_1966} for the 2D bit strings.
The $Z$-order curve is obtained by simply interleaving the binary encoding of the two coordinates.

\subsection{Mapping a Spatial Point to a Bit String Pair}
\label{sec:data-structure:mapping-spatial-point}
A point tuple $(x,y,z)$ at longitude $x$, latitude $y$ and altitude $z$ with
\begin{align*}
...
\right)
\\
\in &\left\{0, 1, ..., 2^{B_x} - 1\right\} \times\\
& \left\{0, 1, ..., 2^{B_y} - 1\right\} \times \\
& \left\{0, 1, ..., 2^{B_z} - 1\right\}
\end{split}
\end{align}


The first term in the longitude and latitude values can be at most $1$ and since $C_x$ and $C_y$ are the maximum values that can be represented using $B_x$ and $B_y$ bits, respectively, the second statement holds for the longitude and latitude.
The statement also holds for the altitude because the subtracting $D$ changes the range to $[0, H - D]$ that after rounding down can by definition be represented using $B_z$ bits.
Let $(\tilde{x}, \tilde{y}, \tilde{z})$ be their big-endian integer binary encoding with
\begin{align*}
	(\tilde{x}, \tilde{y}, \tilde{z}) &\in \{0,1\}^{B_{x}} \times \{0,1\}^{B_{y}} \times \{0,1\}^{B_z}
\end{align*}

For any integer $i \in \{0, ..., B_c-1\}$, let $\tilde{x}_{i}, \tilde{y}_{i}$ and $\tilde{z}_{i}$ denote the $i$-th bit of $\tilde{x}$, $\tilde{y}$ and $\tilde{z}$, respectively, with the first bit being the most significant one.
The bit string pair corresponding of the point $p$ is then defined as
\begin{align}
	\label{eq:definition-bit-string-tuple}
	\left(\left(\concat_{i=0}^{B_{y} - 1} \tilde{x}_{i} || \tilde{y}_{i} \right) || \tilde{x}_{B_{x}-1}, \concat_{i=0}^{B_{z}-1} \tilde{z}_{i}\right)
\end{align}

In words, the bit string pair corresponding to $p$ is a pair of the interleaving of the binary representation for the longitude and latitude coordinates and the binary representation of the altitude.

\Cref{fig:leaf-representation:binary} shows an example of this encoding.
...
\subsection{Geometric Interpretation of Nodes}
\label{sec:data-structure:intermediate}
All nodes in the SMT correspond to a point and a volume.
The point interpretation directly follows from the discretization procedure described in \Cref{sec:data-structure:mapping-spatial-point}, the first entry in the bit string tuple encodes the longitude and latitude and the second entry encodes the altitude.
The volume interpretation is based on this point interpretation.
The volume of maximum length bit string tuples corresponds is the set of all points that would be mapped to it using the discretization procedure.
Due to the floor functions in \Cref{eq:discretization}, the point whose coordinates are encoded by a bit string tuple, is in the corner with the smallest coordinates in all three dimensions inside that volume.
\Cref{fig:leaf-representation:geometric} shows the points and the corresponding volumes in the same color.
For non-maximum length bit string tuples, their volume interpretation is the union of the volumes of all children.

This is now defined explicitly in a formal way by describing how any bit string tuple in $\mathcal{N}$ is interpreted as a point and as a volume.
For any node $n \in \mathcal{N}$, let $(n_0, n_1)$ be the corresponding bit string pair and let $n_{\tilde{x}}, n_{\tilde{y}}$ and $n_{\tilde{z}}$ be the big endian binary encoding of the three coordinates embedded in $n_0$ and $n_1$ corresponding to the longitude, latitude and altitude, respectively.
The values $\tilde{x}$ and $\tilde{y}$ obtained by de-interleaving $n_0$ which starts with an $x$ bit.
The value $\tilde{z}$ is equal to $n_1$.
This is a generalization of \Cref{eq:definition-bit-string-tuple} which defined the format for tree leaves.
Next let $l_{\tilde{x}}$, $l_{\tilde{y}}$ and $l_{\tilde{z}}$ be the lengths of $n_{\tilde{x}}$, $n_{\tilde{y}}$ and $n_{\tilde{z}}$, respectively.
Moreover let $n_{\tilde{x},i}$, $n_{\tilde{y},j}$ and $n_{\tilde{z},k}$ for any $i \in \{0, ..., l_{\tilde{x}}-1\}, j \in \{0, ..., l_{\tilde{y}}-1\}, k \in \{0, ..., l_{\tilde{z}}-1\}$ denote the $i$-th, $j$-th and $k$-th bit of $n_{\tilde{x}}, n_{\tilde{y}}$ and $n_{\tilde{z}}$, respectively.
Then $n_{\hat{x}}$, $n_{\hat{y}}$, $n_{\hat{z}}$ are the most-precise integers that can be re-constructed with the given bit string prefixes:
\begin{align}
	\label{eq:intermediate-geometric-point-partial-height:x}
	n_{\hat{x}} :&= \sum_{i=0}^{l_{\tilde{x}} - 1} 2^{B_{x} - 1 - i} \cdot n_{\tilde{x}, i}\\
	\label{eq:intermediate-geometric-point-partial-height:y}
	n_{\hat{y}} :&= \sum_{i=0}^{l_{\tilde{y}} - 1} 2^{B_{y} - 1 - i} \cdot n_{\tilde{y}, i}\\
	\label{eq:intermediate-geometric-point-partial-height:z}
	n_{\hat{z}} :&= \sum_{i=0}^{l_{\tilde{z}} - 1} 2^{B_{z} - 1 - i} \cdot n_{\tilde{z}, i}
\end{align}
...
n_p :=\left(
n_{\hat{x}} \cdot \frac{360}{C_x} - 180,
n_{\hat{y}} \cdot \frac{180}{C_y} - 90,
D + n_{\hat{z}} \cdot u
\right)
\end{split}
\end{align}
...
n_V :=\left[
n_{\hat{x}} \cdot \frac{360}{C_x} - 180,
(n_{\hat{x}} + 2^{B_{x} - l_{\tilde{x}}}) \cdot \frac{360}{C_x} - 180
\right) &\times\\
\left[
n_{\hat{y}} \cdot \frac{180}{C_y} - 90,
(n_{\hat{y}} + 2^{B_{y} - l_{\tilde{y}}}) \cdot \frac{180}{C_y} - 90
\right) &\times\\
\left[
D + n_{\hat{z}} \cdot u,
D + \left(n_{\hat{z}} + 2^{B_z - l_{\tilde{z}}}
\right) \cdot u\right)&
\end{split}
\end{align}

The $x'_d, y'_d, z'_d$ can also be computed by right padding the bit string to $B_x, B_y$ and $B_z$ bits with zeros and then interpreting the value as an integer.
Similarly the upper bounds for the volume can also be computed by right padding them with ones, interpreting the result as an integer and increasing the value by one.

It is worth nothing is that in GeoPKI, all nodes can store a set of certificates.
When using a tree representing the geography, this is almost a requirement as it allows the placement of certificates covering a large area higher up in the tree.
Otherwise almost no leaf would be empty as most pieces of land on earth are claimed by at least one country.
The resulting tree would certainly not be sparse and likely of infeasible size.

\subsection{Sparse Merkle Hash Tree (SMT)}
\label{sec:data-structure:SMT}

The just described tree is turned into a full SMT by defining the function $\mathcal{H}: \{0,1\}^{*} \times \{0,1\}^{*} \mapsto \{0,1\}^{256}$ mapping each possible bit string pair to a hash value.
For bit string pairs not corresponding to a node in the tree, $\mathcal{H}$ is defined to return a default value, i.e.
\begin{align}
	\forall n \in \left(\{0,1\}^{*} \times \{0,1\}^{*} \right) \backslash \: \mathcal{N}. \: \mathcal{H}(n) := H(\mathbbzero)
\end{align}
where $\mathbbzero$ denotes the zero byte and $H: \{0,1\}^{*} \mapsto \{0,1\}^{256}$ is the SHA-256 hashing function.

The data stored at nodes is a sequence of certificates denoted by $\mathcal{D}: \mathcal{N} \mapsto \left(c_i\right)_{i \in \mathbb{N}}$ where $\mathcal{L}: \mathcal{N} \mapsto \mathbb{N}$ for each nodes denotes the number of certificates in this sequence.
The sequence of certificates is sorted lexicographically by their SHA-256 hashes in ascending order.
The hashes are computed on the PEM encoding of the X.509 certificates.
In contrast to other PKI systems such as F-PKI \cite{chuat_f-pki_2022}, in GeoPKI each node has an associated (possibly empty) sequence of certificates.

If the sequence is indeed empty, the hash value corresponding to that node is also defined to be the same default value, i.e.
\begin{align}
	\forall n \in \{v \in \mathcal{N} \:|\: \mathcal{L}(v) = 0\}. \: \mathcal{H}(n) := H(\mathbbzero)
\end{align}
Hence the hash value of an empty or non-existent node in the SMT is always $H(\mathbbzero)$.

A node $l$ is called a leaf if it does not have any children in the SMT, i.e. is a node whose $z$ bit string is of length $B_z$ (see \Cref{sec:data-structure}).
The hash of a leaf is defined as follows:

$\forall l \in \{v \in \mathcal{N} \: | \: \mathcal{L}(v) \neq 0 \land \mathcal{C}(l) = \emptyset \}$
\begin{align}
\label{eq:hash-leaf}
\begin{split}
	\mathcal{H}(l) &:= H\left(\mathbbzero \: || \: \concat_{i \in \mathcal{L}(l)} H\left(\mathcal{D}(l)_i\right) \right)\\
	\\% 
	&= H\left(\mathbbzero \:||\: H(c_0) \:||\: H(c_1) \:||\: ... \:||\: H(c_{\mathcal{L}(n)-1}) \right)
\end{split}
\end{align}

As usual in a merkle hash tree, for any intermediate node $n := (n_0, n_1)$, the hash function $\mathcal{H}(n)$ takes into account the hashes of $n$'s children.
Let $h_{n,1}, h_{n,2}, h_{n,3}$ and $h_{n,4}$ be the hash values of $n$'s children.
More specifically,

$\forall l \in \{v \in \mathcal{N} \: | \: \mathcal{L}(v) \neq 0 \land \mathcal{C}(l) = \emptyset \}$
\begin{align}
\begin{split}
	h_{n,1} &:= \mathcal{H}\left(\left(n_0 \:||\: 0, n_1\right)\right)
	\\
	h_{n,2} &:= \mathcal{H}\left(\left(n_0 \:||\: 1, n_1\right)\right)
	\\
	h_{n,3} &:= \mathcal{H}\left(\left(n_0, b_1 \:||\: 0\right)\right)
	\\
	h_{n,4} &:= \mathcal{H}\left(\left(n_0, n_1 \:||\: 1\right)\right)
\end{split}
\end{align}

Finally to incorporate the certificate data stored at $l$, hash $h_{n,5}$ is defined as
\begin{align}
	\label{eq:hash-certs-intermediate-node}
	h_{n,5} := H\left(\concat_{i \in \mathcal{L}(n)} H\left(\mathcal{D}(n)_i\right)\right)
\end{align}
Note that $h_{n,5}$ is only well-defined if $\mathcal{L}(n) \neq 0$.
The hash value of the intermediate node is then defined as
\begin{align}
	\label{eq:hash-intermediate-node}
	\mathcal{H}(n) := \left\{
	\begin{array}{lr}
		H(\mathbbone || h_{n,1} || h_{n,2} || h_{n,3} || h_{n,4}) & \text{if $\mathcal{L}(n) = 0$}\\
		H(\mathbbone || h_{n,1} || h_{n,2} || h_{n,3} || h_{n,4} || h_{n,5}) & \text{if $\mathcal{L}(n) \neq 0$}
	\end{array}\right.
\end{align}
where $\mathbbone$ is the byte encoding the integer one, i.e. the byte where only the least significant bit set.

The constants $\mathbbzero$ and $\mathbbone$ are prefixed in the hash computation to prevent collisions where a given hash $H(a || b)$ for some $a$ and $b$ could simultaneously refer to a leaf with content $a || b$ or a intermediate node with children hash values $a$ and $b$.

%\subsection{Querying the SMT}
%\label{sec:data-structure:query}
%
%Since many sizes of volumes can be represented by the binary format described in Sections \ref{sec:data-structure:mapping-spatial-point} and \ref{sec:data-structure:intermediate}, it is for now assumed that the only type of query a GeoPKI map server supports is such a bit string.
...
Let $N \subset \mathcal{N}$ be the set of non-empty nodes whose corresponding volume intersect the query volume.
For each such $n \in N$, the server has to send the set $\{H(c) \: | \: \forall c \in \mathcal{D}(n) \}$ to the client as it is required to compute the node's hash (see Equations \ref{eq:hash-leaf} and \ref{eq:hash-certs-intermediate-node}).
Because parent voxels always cover a super-volume of their children, $N$ includes the transitive closure of the parent relation.
Moreover each parent can have a separate altitude subtree whose intersecting nodes also have to be returned.
For nodes where a child is not in $N$, this child's hash has to be included in the server's response as it is necessary to compute the node's hash as shown in see Equation \ref{eq:hash-intermediate-node}.
The same applies to neighbors, i.e. if the neighbor of some node in $N$ is not also in $N$, its hash has to be included in the server's response since it is required to compute the parent's hash (see \Cref{eq:hash-intermediate-node}).
...
In the worst case there are $B_z \cdot (B_x + B_y)$ nodes intersecting a single point in space, one full altitude subtree path ($B_z$) at each depth of the 2D tree ($B_x + B_y$).
The proof within each altitude subtree requires $B_z - 1$ complementary hashes along the path to the root of the subtree.
In the 2D tree, at most one path of $B_x + B_y$ non-empty nodes will intersect the point resulting in $B_x + B_y - 1$ additional complementary hashes.
Therefore in the worst case a proof will require $(B_x + B_y) \cdot (B_z - 1) + B_x + B_y - 1$ hashes in addition to the retrieved data nodes.
With a hash digest size of $256$ bits or $32$ bytes (SHA-256), the number of hashes alone have a size of $\approx$ 13.5~KiB.
The existence of altitude subtrees at all levels of the 2D tree is very unlikely and in reality the effective proof sizes should be significantly smaller.
% 26 + 25 * (15 - 1) + 26 + 25 - 1 = 426, 426 * 32 = 13'632
Moreover the size of a proof is smaller the sparser the tree is and is always smaller if an intermediate node, i.e., not a leaf, is queried.
...
A query in the database can then be expressed as
...
return int(bit_string[51:].ljust(15, "0"), 2)
\end{lstlisting}

\begin{lstlisting}[language=Python]
# python syntax, ljust = right pad
return int(bit_string[51:].ljust(15, "1"), 2)
\end{lstlisting}
...
Based on the idea of the previous approach of querying a cylinder, it is possible to additionally send a minimum and maximum altitude with a bit string of length $\leq 51$ (whose geometric representation covers the full height).
...
For bit strings longer than $51$ bits only the first $51$ (encoding the longitude and latitude) are taken and for bit strings shorter than $51$, the bits are right padded with zeros up to $51$ bits.
The resulting $51$ bits are then interpreted as a (big endian) integer.
...
\begin{lstlisting}[language=Python]
# python syntax, ljust = right pad
return int(bit_string[:51].ljust(51, c), 2)
\end{lstlisting}
The \texttt{bit\_string\_txt} string column is replaced by a \texttt{bit\_string\_51} bigint column.
...
Since integers are zero extended to $51$ bits, the integer representations of two bit strings is the same if they match when omitting all trailing zeros.
...
\begin{itemize}
	...
	Stores the bit strings described in Sections \ref{sec:data-structure:mapping-spatial-point} and \ref{sec:data-structure:intermediate}.
	There is a non-clustered index on this column enabling fast point queries.
	\item \texttt{bit\_string\_51} of type \texttt{bigint} (64 bit integer)
